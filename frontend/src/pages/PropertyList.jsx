...
const PropertyList = () => {
  ...
  const [filters, setFilters] = useState({ city: '', minRent: '', maxRent: '', bedrooms: '', propertyType: '' });
        ...
        <form onSubmit={handleSearch}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '12px', alignItems: 'end' }}>
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label>City</label>
              <input type="text" placeholder="e.g. Auckland" value={filters.city}
                onChange={(e) => setFilters({ ...filters, city: e.target.value })} />
            </div>
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label>Min Rent ($/week)</label>
              <input type="number" placeholder="e.g. 300" value={filters.minRent}
                onChange={(e) => setFilters({ ...filters, minRent: e.target.value })} />
            </div>
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label>Max Rent ($/week)</label>
              <input type="number" placeholder="e.g. 800" value={filters.maxRent}
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button type="button" className="btn" onClick={handleReset}
                style={{ background: '#f3f4f6', color: '#374151' }}>Reset</button>
            </div>
          </div>
        </form>
      </div>
